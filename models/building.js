...
Building.updateBuilding = (req, result) => {
    const { name, street_number, direction, street_name, street_type, zip, city, state, image, id } = req;
    sql.query('UPDATE buildings SET name = ? street_number = ? direction = ? street_name = ? street_type = ? zip = ? city = ? state = ? image = ?) where id = ?;', [name, street_number, direction, street_name, street_type, zip, city, state, image, id], (err, data) => {
        if (err) {
            result(err, null)
        } else {
            result(null, data)
